...
- 默认可信核验报告与干净 Markdown：每轮调用统一问答后，默认用 `scripts/render_trace_html.py` 生成本轮可信核验报告 HTML（1.3.0 整体重构版：双视图单页+材料专库独立视图、句后引文胶囊原地展开溯源卡、章节引用徽章、只看正文/复制全文/打印归档工具、过程回顾条、生成时链接活性检测与存档快照兜底、段落 id 绑定解析、角标标点换位）和移除角标的同名 `.clean.md`。
