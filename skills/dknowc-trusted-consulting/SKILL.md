...
核验报告规则（1.3.0 整体重构版，对齐公文写作 3.7.0 交互原型）：

- **双视图单页**：核验报告视图（默认）与材料专库视图（全屏）切换并各自记忆滚动位置；顶栏（身份章+标题+只看正文/复制全文/打印归档工具组+阅读进度条）；Hero（大标题+真实统计行+咨询问题行）。
- **句后引文胶囊**：正文为连续文档流（不再分节卡片+右栏面板），角标渲染为胶囊（编号徽章+材料标题），点击原地展开溯源卡（标题链+多段分块原文摘录+查看全文/存档全文/知识专库原文），同段同材料只保留最后一处角标；角标与后随标点整体换位；未绑定角标红色数字警示。
- **材料专库视图**：大搜索（标题/来源/摘录全文检索）+ 热词（真实统计）+ 引用状态筛选（统一问答无检索分组，退化为"全部/未引用"两态，用户问题作为唯一一路检索条件如实展示）。
- **过程回顾条**：可信问答 → 材料入库（答案采用/召回未采用）→ 逐条比对原文 → 核验完成。
- **生成时链接活性检测**：渲染前并发检测材料原文链接，404/410 与软 404 标记失效，连接失败/超时/403 放行（政府站反爬）；`--skip-link-check` 跳过。失效链接优先出"查看存档全文"（接口 screenShotPath），无快照改用知识专库链接（policyUrl）回看，全部缺失如实标注"待补链接"。
- 核验报告单四项指标：依据溯源 / 引用对应 / 材料新旧 / 交付前检查（均由脚本真实计算）；统一问答接口不返回材料类型字段，本 Skill 不设类型标签与材料构成指标；政策现行效力如实列为"现行效力 · 建议人工复核".
...
- 角标编号：渲染时被引用角标按首次出现顺序重编号为 1..n（接口材料自带全量召回序号如 101、601，不直接透传），未引用材料顺延编号；绑定按"段落 id → 材料"优先解析（现接口 content 每段携带全局唯一引文编号，同一材料多段塌缩一张卡），兼容旧形态材料级编号。
