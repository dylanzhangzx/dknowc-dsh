...
深知可信PPT由北京彩智科技有限公司旗下“深知可信智能”提供，是“原生 PPT 生成 + 可信内容层”的演示文稿 Skill：生成侧采用 PPT Master（MIT）的约束 SVG → 原生 DrawingML 编译架构，内容侧完全使用深知可信搜索获取权威、可溯源的素材。
...
├── THIRD_PARTY_NOTICES.md       # ppt-master（MIT）抽取声明
...
│   └── svg_to_pptx 等           # ppt-master（MIT）抽取的编译器组件
...
当前 SkillHub Public 版基于 `1.0.2`：产品更名为「深知可信PPT」（技术标识符不变）；报告统一命名「可信溯源核验报告」；新增提纲版报告——提纲阶段即输出可信溯源核验报告，交付三件套（.pptx + 提纲核验报告 + 成稿核验报告），渲染层零改动。`1.0.1`：优化开通检索的注册引导（价值前置、时机后移、可退路），新增引导参考与效果示例文件，补齐注册请求渠道埋点。
