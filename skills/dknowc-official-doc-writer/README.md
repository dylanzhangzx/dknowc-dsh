...
当前 dsh 版基于母版 `3.7.0`：溯源核验报告整体重构（参考深知晓可信深度溯源报告原型——双视图单页/句后引文胶囊/材料专库独立视图/章节引用徽章/只看正文·复制全文·打印归档工具/过程回顾条/生成时链接活性检测与存档快照兜底/核验结论分级/发文字号关键性行/多段分块面包屑标题链）；核验结论分级（缺链黄提醒不拖垮结论）；自由搜索（webSearch）文风体例参考专用规范。dsh 侧：**公文范文大纲改为 MCP 第四工具 `doc_outline` 转接**（适配层 outline 模式与 outline_reference 同构，直连脚本降为兜底）；全部脚本重打会话隔离补丁；注册统一标记块。
