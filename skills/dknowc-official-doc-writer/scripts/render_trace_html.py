...
三个核验层次：
- 报告级：核验报告单（依据溯源 / 引用绑定 / 时效检查 / 类型覆盖 / 成稿自检）
...
def compute_verification(answer: str, sources: List[Dict[str, str]], payload: Dict[str, Any],
    ...
    trace_ok = sources and trace_passed == len(sources)
...
def render_verify_panel(v: Dict[str, Any]) -> str:
    ...
    if sc["status"] == "pass":
        ...
        sc_html = (f'<div class="vi"><span class="s ok"{title_attr}>✓ 成稿自检 {sc["passed"]}/{sc["total"]}</span>'
                   f'<span class="d">事实/结构/占位/AI味/格式{esc(" · 悬停查看核验说明" if notes else "")}</span></div>')
    elif sc["status"] == "fail":
        failed_parts = []
        ...
        sc_html = (f'<div class="vi"><span class="s fail">✗ 成稿自检 {sc["passed"]}/{sc["total"]}</span>'
                   f'<span class="d">未通过：{esc("、".join(failed_parts))}</span></div>')
    else:
        sc_html = '<div class="vi"><span class="s none">— 成稿自检 未记录</span><span class="d">本次溯源 JSON 未写入自检结果</span></div>'
    ...
    <div class="verify {state}">
      ...
      <div class="v-note">核验方法：深知可信搜索召回权威来源 → 逐条溯源（摘录比对 + 原文链接 + 知识专库回看）→ 正文引用绑定 → 成稿自检。政策现行效力以官方发布为准。{manual_checks_html}</div>
    </div>"""
...
<div class="layout">
  <main class="report" aria-label="核验报告正文">
    ...
    <div class="foot">深知公文写作 · 可信溯源核验报告 ｜ 核验方法：可信搜索召回 → 逐条溯源 → 正文绑定 → 成稿自检 ｜ 内容由 AI 生成，仅供参考，政策现行效力以官方发布为准</div>
  </main>
...
def main() -> None:
    # dsh：未显式传 --output 时，默认输出到会话产物目录 official-docs/output/
    import os as _os
    _ws = _os.environ.get("DKNWOC_WS_ROOT")
    if not _ws:
        _sid = _os.environ.get("DSH_SESSION_ID", "")
        _ws = str(Path.cwd() / "dknowc-output" / (_sid[:8] if _sid else "_default"))
    default_output_dir = str(Path(_ws).resolve() / "official-docs" / "output")
