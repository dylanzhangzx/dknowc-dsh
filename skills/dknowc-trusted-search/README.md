...
- 完整返回：`trusted_search.py` 默认 `simplified=false`（精简集会丢存档快照，生成核验报告时不要用 `--simplified`）。
- 最终交付：直接回复答案、可信溯源核验报告 HTML、无来源角标的干净 Markdown。
- 默认核验报告：`render_trace_html.py` 整体重构版——双视图单页（核验报告视图+材料专库独立视图）、句后引文胶囊原地展开溯源卡、章节引用徽章、只看正文/复制全文/打印归档工具、过程回顾条、生成时链接活性检测（--skip-link-check 跳过）与存档快照兜底（--no-snapshot 关闭）、核验结论分级（缺链黄提醒不拖垮结论、缺摘录计未通过）、五项指标真实计算、角标自动重排；`--self-check-file` 传入答案自检结果。
